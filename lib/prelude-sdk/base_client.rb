...
module PreludeSDK
  ...
  class BaseClient
    ...
    #
    #   @option req [PreludeSDK::Converter, Class, nil] :model
    #
    ...
    private def send_request(request, redirect_count:, retry_count:, send_retry_header:)
      ...
      in (400..) if retry_count >= max_retries || (response && !self.class.should_retry?(
        ...
        decoded = PreludeSDK::Util.decode_content(response, stream: stream, suppress_error: true)
    ...
    private def parse_response(req, headers:, stream:)
      decoded = PreludeSDK::Util.decode_content(headers, stream: stream)
      unwrapped = PreludeSDK::Util.dig(decoded, req[:unwrap])

      case [req[:page], req.fetch(:model, PreludeSDK::Unknown)]
      in [Class => page, _]
        page.new(client: self, req: req, headers: headers, unwrapped: unwrapped)
      in [nil, Class | PreludeSDK::Converter => model]
