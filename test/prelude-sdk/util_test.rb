...
class PreludeSDK::Test::UtilUriHandlingTest < Minitest::Test
  ...
  def test_joining
    ...
    cases.each do |expect, lhs, rhs|
      assert_equal(
        URI.parse(expect),
        PreludeSDK::Util.join_parsed_uri(
          PreludeSDK::Util.parse_uri(lhs),
          rhs
        )
      )
    end
  ...
  def test_joining_queries
    ...
    cases.each do |path, expected|
      assert_equal(
        URI.parse(expected),
        PreludeSDK::Util.join_parsed_uri(
          PreludeSDK::Util.parse_uri(base_url),
          {path: path}
        )
      )
    end
...
class PreludeSDK::Test::UtilFormDataEncodingTest < Minitest::Test
  ...
  def test_file_encode
    ...
    cases.each do |body, val|
      encoded = PreludeSDK::Util.encode_content(headers, body)
...
class PreludeSDK::Test::UtilFusedEnumTest < Minitest::Test
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = PreludeSDK::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = PreludeSDK::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_close_fused
    ...
    PreludeSDK::Util.close_fused!(fused)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = PreludeSDK::Util.fused_enum(enum)
    first = fused.next
  ...
  def test_close_fused_sse_chain
    ...
    fused_1 = PreludeSDK::Util.fused_enum(enum)
    fused_2 = PreludeSDK::Util.decode_lines(fused_1)
    fused_3 = PreludeSDK::Util.decode_sse(fused_2)

    assert_equal(0, taken)
    PreludeSDK::Util.close_fused!(fused_3)
    assert_equal(0, taken)
  end
end
...
class PreludeSDK::Test::UtilSseTest < Minitest::Test
  def test_decode_lines
    ...
    cases.each do |enum, expected|
      eols.each do |eol|
        lines = PreludeSDK::Util.decode_lines(enum.map { _1.gsub("\n", eol) })
  ...
  def test_mixed_decode_lines
    ...
    cases.each do |enum, expected|
      lines = PreludeSDK::Util.decode_lines(enum)
  ...
  def test_decode_sse
    ...
    cases.each do |name, test_cases|
      test_cases.each do |input, expected|
        actual = PreludeSDK::Util.decode_sse(input).map(&:compact)
